...
from deploy_contract import deploy, update
from call_contract import invoke, call
...
class Command:
    ...
    def optimizedJar(args):
        ...
        build_result = str(bash(f"./gradlew {javaPkg}:build"))

        if not "BUILD SUCCESSFUL" in build_result:
            print(build_result)

        print(f"Executing {javaPkg}:optimizedJar ...")

        optimizedResult = str(bash(f"./gradlew {javaPkg}:optimizedJar"))
        if not "BUILD SUCCESSFUL" in optimizedResult:
